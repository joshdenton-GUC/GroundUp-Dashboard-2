-- =============================================================================
-- COMPLETE FIX FOR PROFILE SIGNUP ISSUE
-- =============================================================================
-- This script does 3 things:
-- 1. Fixes the trigger for future signups
-- 2. Backfills missing client records for existing users
...
INSERT INTO public.clients (user_id, company_name, contact_phone)
SELECT
  p.user_id,
  COALESCE(
...
-- Check 1: Verify the trigger function exists
SELECT
...
-- Check 3: Count profiles vs clients to ensure they match
SELECT
  ...
  CASE
    ...
    ELSE '⚠️ Some profiles missing client records'
  END as status;

-- Check 4: List any profiles still missing client records (should be empty)
SELECT
  p.user_id,
  p.email,
  p.full_name,
